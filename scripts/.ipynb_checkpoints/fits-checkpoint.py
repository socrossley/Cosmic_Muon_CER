...
def get_inputs():
    ...
    parser.add_argument("-s", "--save", default='', help="Save to file in \'./data/fit_data/th\'")
...
def get_theoretical_fit_data(num_bins, min_e=1, max_e=10, spread_pct=0):
    ...
    left_bin_edge_padding = 2 
    right_bin_edge_padding = 10
...
def main():
    args = get_inputs()
    df = get_theoretical_fit_data(args['num_bins'], spread_pct = args['error'])
    if args['save']:
        print(f'Saving to {args["save"]}')
        df.to_csv(r'../data/fit_data/th/' + args['save'], index=False, header=True)
    print('Saved!')
